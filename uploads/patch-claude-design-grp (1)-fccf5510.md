...
## 9. Botão "Entrar" (em breve)

No cabeçalho, mantenha o texto **"Entrar"**. O sistema ainda não tem endereço público, então **não construa tela de login** e não aponte para nenhum domínio. Deixe o link como `#` com `title="Em breve"` e um estilo levemente apagado (menor destaque), sinalizando que ainda não está ativo. Quando o `app` existir, será só trocar o `href`.

## 10. Canal de contato: formulário + WhatsApp

A página de demonstração mantém o **formulário de 7 campos** como está. Confirme que o botão **"Falar no WhatsApp"** aparece tanto na página de demonstração quanto no CTA final da home, com link `https://wa.me/[NUMERO]` e mensagem pré-preenchida: *"Olá, vim pelo site do GRP e quero uma demonstração."*

O número do WhatsApp e o e-mail de destino do formulário entram como `[A DEFINIR]` até a equipe informar — deixe visível, não invente.

---

**Não altere:** a estrutura do formulário de 7 campos, a estrutura das páginas legais, os placeholders `[A DEFINIR]`, nem a caixa de honestidade sobre anonimato.
